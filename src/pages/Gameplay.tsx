...
type Phase = 'loading' | 'question' | 'locked' | 'correct-prompt' | 'lifeline-audience' | 'lifeline-friend' | 'finishing'
...
export default function Gameplay() {
      ...
      window.setTimeout(() => {
        if (correct) {
          ...
          if (currentLevel >= LADDER.length) {
            window.setTimeout(() => finishGame('won', pointsForLevel(currentLevel), currentLevel, nextAnswers), 900)
          } else {
            const isCheckpoint = LADDER.find((l) => l.level === currentLevel)?.isCheckpoint
            if (isCheckpoint) sound.playCheckpoint()
            setPhase('correct-prompt')
          }
        } else {
          sound.playWrong()
          window.setTimeout(
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      setTimeLeft((s) => s - 1)
      if (timeLeft <= 6) sound.playCountdownBeep()
    }, 1000)
  ...
  const dismissLifelinePanel = () => setPhase('question')
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xl font-bold">{config.playerName}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-sm text-white/60">Secured</p>
              <p className="text-lg font-bold text-amber-300">{secured.toLocaleString()} 👑</p>
            </div>
        ...
        {phase === 'question' && (
          <TimerBar timeLeft={timeLeft} total={config.timerSecondsPerQuestion} />
        )}

        <div className="rounded-3xl bg-gradient-to-br from-purple-800/60 to-indigo-900/60 p-6 shadow-xl">
          <div className="mb-3 flex items-center gap-2 text-xs">
            <span className="rounded-full bg-black/30 px-3 py-1 font-bold">Level {currentLevel} of {LADDER.length}</span>
            <span className="rounded-full bg-black/30 px-3 py-1">{currentQuestion.category}</span>
          </div>
          <p className="text-2xl font-bold leading-snug sm:text-3xl">{currentQuestion.text}</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {currentQuestion.options.map((opt, i) => {
            ...
            const showResult = phase === 'locked' || phase === 'correct-prompt'

            let stateClasses = 'bg-white/10 hover:bg-white/20'
            if (isDisabled) stateClasses = 'bg-white/5 opacity-30 line-through'
            if (phase === 'locked' && isSelected && !showResult) stateClasses = 'bg-amber-400/60 animate-pulse-glow'
            if (showResult) {
              if (isCorrectAnswer) stateClasses = 'bg-green-500/80 ring-4 ring-green-300'
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-2xl px-5 py-4 text-left text-lg font-semibold transition ${stateClasses} ${
                  phase === 'question' && !isDisabled ? 'cursor-pointer' : ''
                }`}
              >
        ...
        <div className="flex flex-wrap gap-3">
          <LifelineButton
            ...
            available={config.lifelines.fiftyFifty}
        ...
        {phase === 'locked' && timedOut && (
          <p className="text-center text-lg font-bold text-red-300">⏰ Time's up!</p>
        )}
        {phase === 'locked' && currentQuestion.funFact && (
          <p className="rounded-xl bg-black/20 p-3 text-sm text-white/70">💡 {currentQuestion.funFact}</p>
        )}

        {phase === 'correct-prompt' && (
          <div className="rounded-2xl bg-green-900/40 p-5 text-center">
            <p className="mb-3 text-lg font-bold">
              ✅ Correct! {config.playerName} has secured {pointsForLevel(currentLevel).toLocaleString()} 👑
            </p>
            {currentQuestion.funFact && <p className="mb-3 text-sm text-white/70">💡 {currentQuestion.funFact}</p>}
            <div className="flex justify-center gap-3">
              <button
                onClick={handleContinue}
                className="rounded-xl bg-amber-400 px-6 py-3 font-bold text-purple-950 hover:scale-105 transition"
              >
                Continue to Level {currentLevel + 1} →
              </button>
              <button onClick={handleWalkAwayAfterCorrect} className="rounded-xl bg-white/10 px-6 py-3 font-bold hover:bg-white/20">
        ...
        {phase === 'lifeline-friend' && friendHint && (
          <LifelinePanel onDismiss={dismissLifelinePanel} title="📞 Phone a Friend">
            <p className="text-lg">
              "{friendHint.line} <span className="font-bold text-amber-300">{optionLabel(friendHint.index)} — {currentQuestion.options[friendHint.index]}</span>"
            </p>
          </LifelinePanel>
        )}
      </div>
...
function TimerBar({ timeLeft, total }: { timeLeft: number; total: number }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <p className={`text-center text-sm font-bold ${urgent ? 'text-red-400' : 'text-white/60'}`}>{timeLeft}s</p>
    </div>
  )
}
...
function LifelinePanel({ title, children, onDismiss }: { title: string; children: React.ReactNode; onDismiss: () => void }) {
  return (
    <div className="rounded-2xl bg-indigo-950/60 p-5">
      <h3 className="mb-3 text-lg font-bold">{title}</h3>
      {children}
      <button onClick={onDismiss} className="mt-4 w-full rounded-xl bg-amber-400 py-2 font-bold text-purple-950">
